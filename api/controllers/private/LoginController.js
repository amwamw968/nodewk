...
module.exports = {
  ...
  doLogin: function (req, res) {
    ...
    req.session.errorCount = errCount + 10;
	console.error('username = ', username);
    Sys_user.findOne({loginname: username}).populate('roles', {disabled: false}).exec(function (err, user) {
	console.error('find name: ===, user=', user);
	console.error('find name: ===, err=', err);
      if (!user || err) {
	console.error('no user name');
	return res.json({code: 3, msg: sails.__('private.login.nousername')});
      }
